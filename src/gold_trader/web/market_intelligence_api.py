...
<body style="margin:0;background:#06090d;color:#eef3fb"><div id="root"></div><script>
const TFs=['D1','H4','H1','M30','M15','M5','M1'];
let state={page:'trade',tf:'M15',decision:null,health:null,candles:[],candleMeta:null,loadError:null};
const $=s=>document.querySelector(s);
function esc(s){return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;')}
function itemText(x){return typeof x==='string'?x:JSON.stringify(x)}
...
function actionLabel(a){a=String(a||'WAIT').toUpperCase(); if(a.indexOf('HARD')>=0) return 'WAIT HARD BLOCK'; if(a.indexOf('TRADE_READY')>=0) return 'PAPER TRADE READY'; return a.replace(/_/g,' ')}
async function j(url){const r=await fetch(url,{cache:'no-store'}); if(!r.ok) throw new Error(url+' '+r.status); return r.json()}
function renderLoading(){document.getElementById('root').innerHTML='<div class="app" style="min-height:100vh;display:grid;place-items:center;background:#06090d;color:#eef3fb;font-family:system-ui,sans-serif"><div style="text-align:center"><h2 style="letter-spacing:.16em;margin:0 0 12px">Gold Trader</h2><p style="color:#92a0b2;margin:0">Loading command center…</p></div></div>'}
async function load(){if(!state.decision) renderLoading(); state.loadError=null; try{state.decision=await j('/api/decision');state.health=await j('/api/provider-health')}catch(e){state.loadError=String(e); console.error(e)} render(); void loadCandles(state.tf)}
...
function lists(d){return `<div class="right"><div class="panel control"><h4>Operator Control</h4><div class="body"><b>${d.live_orders_enabled?'LIVE ORDERS OPEN':'LIVE ORDERS LOCKED'}</b><p>${d.live_orders_enabled?'Execution is enabled. Confirm broker/spread before firing.':'Paper/alert only. Live execution cannot fire from this UI.'}</p></div></div><div class="panel"><h4>Watching For</h4><div class="body"><ul>${(d.watching_for||[]).map(x=>`<li>${esc(itemText(x))}</li>`).join('')||'<li>No active checklist</li>'}</ul></div></div><div class="panel"><h4>Why</h4><div class="body"><ul>${(d.readable_reasons||d.reasons||[]).map(x=>`<li>${esc(itemText(x))}</li>`).join('')||'<li>No items</li>'}</ul></div></div><div class="panel"><h4>Blockers</h4><div class="body"><ul>${(d.readable_blockers||d.blockers||[]).map(x=>`<li>${esc(itemText(x))}</li>`).join('')||'<li>No blockers</li>'}</ul></div></div>${contextPanel(d)}</div>`}
...
function render(){const d=state.decision||{}; let body=''; if(state.loadError) body=`<section class="card"><h3>Load error</h3><div class="body" style="padding:18px"><p class="danger">${esc(state.loadError)}</p><button class="chip ok" onclick="load()">Retry</button></div></section>`; else { if(state.page==='trade')body=trade(d); if(state.page==='market')body=market(d); if(state.page==='signal')body=signal(d); if(state.page==='risk')body=risk(d); if(state.page==='journal')body=journal(d); if(state.page==='settings')body=settings(d); if(state.page==='json')body=`<section class="card"><h3>Decision JSON</h3><pre class="json">${esc(JSON.stringify(d,null,2))}</pre></section>` } try{document.getElementById('root').innerHTML=`<div class="app">${top(d)}${body}</main></div>`; drawSoon()}catch(e){document.getElementById('root').innerHTML=`<div style="padding:40px;color:#ff5770;font-family:monospace">Render error: ${esc(String(e))}</div>`}}
renderLoading(); load(); setInterval(load,15000);
</script></body></html>'''
