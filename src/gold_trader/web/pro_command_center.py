...
FRONTEND_DIR = ROOT / "frontend" / "pro_command_center"
...
def _apply_pro_fields(base: dict) -> dict:
    action = str(base.get("action") or "WAIT").upper()
    if "TRADE_READY" in action:
        base["verdict_label"] = "PAPER TRADE READY"
    elif action in {"BUY", "SELL"}:
        base["verdict_label"] = action
    elif action == "WAIT":
        base["verdict_label"] = "WAIT"
    else:
        base["verdict_label"] = action.replace("_", " ")

    base["is_trade_ready"] = "TRADE_READY" in action or action in {"BUY", "SELL"}
    base["orders_locked"] = os.getenv("GOLD_ENABLE_LIVE_ORDERS", "false").lower() != "true"
    base["execution_mode"] = os.getenv("GOLD_EXECUTION_MODE", "paper")
    base["data_provider"] = os.getenv("GOLD_MARKET_DATA_PROVIDER", "twelvedata")
    base["timeframe_reads_ordered"] = base.get("timeframe_reads") or []
    return base
...
def load_decision() -> dict:
    ...
    base = _apply_pro_fields(base)

    cloud = dict(base.get("cloud_status") or {})
    cloud.setdefault("data_provider", base.get("data_provider") or os.getenv("GOLD_MARKET_DATA_PROVIDER", "twelvedata"))
...
class Handler(BaseHTTPRequestHandler):
    server_version = "GoldTraderProCommandCenter/1.0"
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/api/candles":
            ...
            if not candles:
                payload["error"] = provider if "error" in provider else "No candle data returned"
            self._json(payload)
            return
    ...
    def log_message(self, fmt: str, *args: object) -> None:
        print(f"[pro-cc] {self.address_string()} {fmt % args}", flush=True)
...
def serve(host: str | None = None, port: int | None = None) -> None:
    ...
    if not FRONTEND_DIR.exists():
        raise FileNotFoundError(f"Pro command center frontend not found at {FRONTEND_DIR}")
    ...
    print(f"[pro-cc] serving on {host}:{port} from {FRONTEND_DIR}", flush=True)
    httpd.serve_forever()
