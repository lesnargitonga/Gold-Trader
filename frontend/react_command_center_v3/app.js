(() => {
  ...
  function App() {
    const [page, setPage] = useState("Trade Cockpit"), [d, setD] = useState(null), [err, setErr] = useState(null);
    const load = () => getJSON("/api/decision").then(setD).catch((e) => setErr(e.message));
    useEffect(() => {
      load();
